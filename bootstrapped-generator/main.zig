...
pub fn main(init: std.process.Init) !void {
    ...
    var reader = stdin.reader(init.io, &.{});
    const file_buffer = try reader.interface.allocRemaining(allocator, .limited(buffer_size));

    const request: plugin.CodeGeneratorRequest = try .decode(file_buffer, allocator);

    var ctx: GenerationContext = .init(init.io, init.environ_map, allocator, request);
...
const GenerationContext = struct {
    ...
    fqn_lines: std.StringHashMap(std.ArrayListUnmanaged([]const u8)),

    /// map of message names to their dependencies
    message_deps: std.StringHashMap(std.ArrayListUnmanaged([]const u8)),

    pub fn init(io: std.Io, env_map: *std.process.Environ.Map, allocator: std.mem.Allocator, request: plugin.CodeGeneratorRequest) GenerationContext {
        return .{
            .io = io,
            .env_map = env_map,
            .req = request,
    ...
    fn getOutputLines(self: *GenerationContext, allocator: std.mem.Allocator, name: FullName) !*std.ArrayListUnmanaged([]const u8) {
        ...
        if (!entry.found_existing) {
            ...
            while (it.next()) |package| {
                if (!std.mem.eql(u8, package.key_ptr.*, name.buf)) {
                    try lines.append(allocator, try std.fmt.allocPrint(allocator, "/// import package {s}\n", .{package.key_ptr.*}));
    ...
    fn resolvePath(self: *GenerationContext, allocator: std.mem.Allocator, a: []const u8, b: []const u8) ![]const u8 {
        ...
        const resolvedRelativePath = try std.fs.path.relative(allocator, cwd, self.env_map, aPath, bPath);
    ...
    fn generateFieldDescriptor(
        ...
        try lines.append(allocator, try std.fmt.allocPrint(allocator, format, .{ name, field.number, descStr }));
    }
    ...
    fn generateFieldDeclaration(
        ...
        if (try self.getFieldDefault(allocator, field, file, nullable)) |default_value| {
            try lines.append(allocator, try std.fmt.allocPrint(allocator, "    {s}: {s} = {s},\n", .{ field_name, type_str, default_value }));
        } else {
    ...
    fn generateMessages(
        ...
        for (messages.items) |message| {
            ...
            for (m.oneof_decl.items, 0..) |oneof, i| {
                ...
                if (union_element_count > 1) {
                    const oneof_name = oneof.name.?.getSlice();
                    try lines.append(allocator, try std.fmt.allocPrint(allocator, "    .{s} = fd(null, .{{ .OneOf = {s}_union }}),\n", .{ oneof_name, oneof_name }));
                }
            }
...
pub fn formatSliceEscapeImpl(allocator: std.mem.Allocator, str: []const u8) ![]const u8 {
    ...
    try out.print(allocator, "\"{}\"", .{std.zig.fmtString(str)});
